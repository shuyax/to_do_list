...
function addNewToDoToProject(project,to_do_index) {
    ...
    if (user_description == '') {
        ...
        if (current_project.searchToDo(user_description) == false){
            ...
            updateUIToDoTask(to_do_index,user_description,user_due,user_priority,user_status)
            return true
        } else {
...
function updateUIToDoTask(to_do_index,user_description,user_due,user_priority,user_status){
    const table_body = document.querySelector('.table_body')
    const li = document.getElementById(to_do_index)
