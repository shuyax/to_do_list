...
export class Project {
  ...
  getToDosByDue(due_start, due_end) {
    ...
    const filtered_todos = this.project_to_do.filter((to_do) => {
      return (
        to_do.due_date_time.getTime() <= due_end.getTime() &&
        to_do.due_date_time.getTime() >= due_start.getTime()
      );
    });
  ...
  getToDosByPriority(priority_value) {
    // eslint-disable-next-line prefer-const, arrow-body-style
  ...
  getToDosByStatus(status_value) {
    // eslint-disable-next-line arrow-body-style, prefer-const
    let projectToDos = this.project_to_do.filter((to_do) => {
  ...
  deleteToDo(description_value) {
    ...
    const delete_index = this.projectToDo.findIndex(
...
const homeRenovationToDo = [
  new ToDo(
    "Purchase paint for living room",
  ...
  new ToDo(
    "Hire plumber for bathroom repairs",
    new Date(2024, 3, 12, 15, 0, 0),
    "high",
    "doing",
  ),
];
export const homeRenovation = new Project(
  "Home Renovation",
  homeRenovationToDo,
);
...
const studyPlanToDo = [
  new ToDo(
    "Read Chapter 1 of Biology textbook",
...
export const vacationPlanning = new Project(
  "Vacation Planning",
  vacationPlanningToDo,
);
...
export const newProjectB = new Project("   project B  ", [
  new ToDo("description 1", new Date(2024, 2, 20, 23, 0, 0), "normal", "to_do"),
  new ToDo("description 2", new Date(2024, 3, 20, 15, 0, 0), "low", "to_do"),
]);
export const newProjectC = new Project("project C  ", [
  new ToDo("description a", new Date(2024, 2, 25, 3, 0, 0), "high", "done"),
  new ToDo("description c", new Date(2024, 2, 21, 3, 0, 0), "high", "doing"),
  new ToDo("description b", new Date(2024, 2, 23, 3, 0, 0), "low", "to_do"),
]);
